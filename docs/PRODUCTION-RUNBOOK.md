...
1. **误报**：`FETCH_ERR`、以及 HTTP **401 / 403 / 405** 常见于反爬、地区墙或本机 TLS/代理问题，**不等于**站点已死。大牌站（GitHub、云厂商控制台等）优先用浏览器手点确认，再在 Admin 决定 resolve 或改 URL。
2. **节奏**：建议**每周**跑一轮 `check:links --json --persist`（service role 指向 nav-prod），随后在 Admin「链接健康」处理 open 队列；不必日更除非上线大批量新链。
3. **原则**：**不**自动下架、**不**自动改 URL。恢复正常的站可 `resolve` 关闭 finding；确认真 **404 / 410** 或域名失效后再人工改 URL 或下架。队列产品化已交付；检测算法与本手册 §2 CLI 保持不变。
