...
| 新一轮规划单独使用带日期的主计划 | 保留 7 月 4 日性能专项历史，避免把发布计划与已完成性能任务混为一谈 |
| 候选证据按 release manifest 管理 | 测试、迁移、部署和回滚必须绑定同一不可变 SHA |
| 当前架构保持模块化单体 | 没有独立扩缩容、团队所有权或 SLA 证据支持拆微服务 |

## 遇到的错误
| 错误 | 尝试次数 | 解决方案 |
|------|---------|---------|
| 嵌套 PowerShell/RTK 的变量和数组引号被外层解析 | 2 | 停止批量嵌套脚本，改用无变量的独立 `Test-Path` 检查 |
| 普通 `git diff --stat` 不显示未跟踪的新计划文档 | 1 | 结合 `git status --short`、`Test-Path` 和显式内容检查 |
