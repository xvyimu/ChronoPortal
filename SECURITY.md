...
- 公开 API 有速率限制（submit 15min/3次, click 15min/1次, login 15min/5次；search/favicon 另有进程内限流）
- 管理员密码优先 `ADMIN_PASSWORD_HASH`（scrypt）；过渡期明文 `ADMIN_PASSWORD` 使用 `timingSafeEqual`
- 生成哈希：`pnpm hash:admin-password` / `node scripts/hash-admin-password.mjs`
